...
export default function AmbassadorLoginPage() {
  ...
  useEffect(() => {
    if (user) router.replace(user.role === 'ambassador' ? '/ambassador' : `/${user.role}`);
  }, [user, router]);
      ...
      <div className="w-full max-w-md animate-fade-in">
        <div className="rounded-xl border border-gray-200 bg-white p-8 shadow-sm">
          <div className="flex items-center justify-center mb-2">
            <span className="flex h-14 w-14 items-center justify-center rounded-xl bg-cyan-50 text-3xl">🚀</span>
          </div>
          <h1 className="text-3xl font-bold text-center text-gray-900">Ambassador Sign In</h1>
          <p className="mt-2 text-center text-gray-500">Complete quests and earn XLM rewards</p>
